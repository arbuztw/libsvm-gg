...
void knkmeans(double **K, const int *indices, const int msize, const int ncluster, const int max_iter,
		double *cluster_avg, int *cluster, int *csize)
{
	int *label = Malloc(int, msize);
	int *current = Malloc(int, ncluster);
	...
	for (int iter = 0; iter < max_iter + 1; iter++) {
		...
		for (int i = 0; i < ncluster; i++) {
			...
			for (int j = start; j < start+csize[i]; j++) {
				...
				for (int k = j + 1; k < start+csize[i]; k++) {
					int idx_k = cluster[k];
...
void leveled_knkmeans(const svm_parameter *param, const svm_problem *prob, const int lvl, const int nchild, int **full_cidx, int **full_csize)
{
	...
	svm_node **sample = Malloc(svm_node*, 2000);

	for (int i = 0; i < prob->l; i++)
		perms[i] = i;
	std::random_shuffle(perms, perms+prob->l);
	...
	knkmeans_predict_alllevel(param, prob, sample, sub_cidx, sub_csize, msize, cluster_avg, same_cluster_map, lvl, nchild, full_cidx, full_csize);

	free(sample);

	for (int i = 0; i < msize; i++) free(Ksample[i]);
	free(Ksample);
